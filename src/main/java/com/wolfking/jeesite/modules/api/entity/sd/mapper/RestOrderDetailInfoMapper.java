...
public class RestOrderDetailInfoMapper extends CustomMapper<RestOrderDetailInfo, Order>{
    ...
    public void mapBtoA(Order b, RestOrderDetailInfo a, MappingContext context) {
        ...
        a.setServiceAddress(AreaUtils.getCityCountyName(condition.getArea().getName()) + condition.getServiceAddress());
        ...
        Long customerId = Optional.ofNullable(condition.getCustomer()).map(t->t.getId()).orElse(0L);
        ...
        if(null == materials || 0 == materials.size()){
            ...
            //配件与返件单资料分开存取
            //step1:未审核的配件单/返件单
            long count = materials.stream().filter(t-> t.getStatus().getValue().equals("1")).count();
            if ( count > 0 ){
                a.setPartsStatus(1);
            }else{
                //step2:返件，未发货
                if(context !=null) {
                    List<MaterialReturn> returnMaterials = (List<MaterialReturn>) context.getProperty("returnMaterials");
                    if (!CollectionUtils.isEmpty(returnMaterials)) {
                        count = returnMaterials.stream().filter(t -> ArrayUtil.contains(new String[]{"1","2"},t.getStatus().getValue())).count();
                        if (count > 0) {
                            a.setPartsStatus(1);
                        }
                    }
        ...
        if (b.getOrderAdditionalInfo() != null && StringUtils.isNotBlank(b.getOrderAdditionalInfo().getEstimatedReceiveDate())) {
            ...
            a.setBuyDate(b.getOrderAdditionalInfo().getBuyDate());
        }
