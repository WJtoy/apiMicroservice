...
//    @Value("${shortmessage.ignore-data-sources}")
     ...
     * @param user
     * @param orderRequest
     * @return public RestResult<Object> saveAppAppointmentDate(HttpServletRequest request,User user, @RequestBody RestSetAppointmentDateRequest orderRequest){
    ...
    public RestResult<Object> saveAppAppointmentDate(HttpServletRequest request, User user, @RequestBody RestSetAppointmentDateRequest orderRequest) {
        ...
        try {
            if (user == null) {
                ...
                boolean aBoolean = VerificationUtils.compare(
                        orderId,orderRequest.getQuarter(), OperationCommand.OperationCode.SET_APPOINTMENT_DATE.code,userInfo.getEngineerId(),userInfo.getServicePointId());
                if (!aBoolean){
                    return RestResultGenerator.custom(ErrorCode.ORDER_CAN_NOT_APPOINTMENT.code, ErrorCode.ORDER_CAN_NOT_APPOINTMENT.message);
                }
...
//            }
            //短信 原代码提炼为方法
            sendAppPendingSMS(order.getDataSource().getIntValue(), pendingType.getIntValue(), orderCondition.getServicePhone(), userId, (new Date().getTime()));
//            ignoreDataSources.clear();
    ...
    public RestResult<Object> saveAppComplete(HttpServletRequest request, User user, RestCloseOrderRequest orderRequest) {
        ...
        try {
            ...
            //}
            Order order = orderService.getOrderById(orderId, orderRequest.getQuarter(), OrderUtils.OrderDataLevel.DETAIL, true,true,true);
            if (order == null || order.getOrderCondition() == null) {
                return RestResultGenerator.custom(ErrorCode.DATA_PROCESS_ERROR.code, "读取订单:" + ErrorCode.DATA_PROCESS_ERROR.message);
            }
            if (order.getDataSourceId() == B2BDataSourceEnum.VIOMI.id) {
                if (orderRequest.getBuyDate() == null || orderRequest.getBuyDate() == 0) {
                    return RestResultGenerator.custom(ErrorCode.DATA_PROCESS_ERROR.code, "错误：请选择购买时间");
                }
                if (order.getOrderCondition().getCreateDate() == null || orderRequest.getBuyDate() > order.getOrderCondition().getCreateDate().getTime()) {
                    return RestResultGenerator.custom(ErrorCode.DATA_PROCESS_ERROR.code, "错误：购买时间不允许晚于下单时间");
                }
            }
            if (order.getOrderCondition().getOrderServiceType() == OrderUtils.OrderTypeEnum.BACK.getId()
                    || order.getOrderCondition().getOrderServiceType() == OrderUtils.OrderTypeEnum.EXCHANGE.getId()) {
                return RestResultGenerator.custom(ErrorCode.DATA_PROCESS_ERROR.code,"错误：退货或换货工单请联系客服处理！");
            }
            OrderCondition condition = order.getOrderCondition();
            if (condition.getServicePoint() == null || condition.getEngineer() == null) {
                return RestResultGenerator.custom(ErrorCode.MEMBER_ENGINEER_NO_EXSIT.code, "读取师傅信息失败");
            ...
            if (order.isSuspended() == 1) {
                return RestResultGenerator.custom(ErrorCode.DATA_PROCESS_ERROR.code,"错误：挂起工单不允许进行操作，请联系客服处理！");
            }
            engineer = servicePointService.getEngineerFromCache(condition.getServicePoint().getId(), condition.getEngineer().getId());
            if (null == engineer) {
                return RestResultGenerator.custom(ErrorCode.MEMBER_ENGINEER_NO_EXSIT.code, "读取师傅信息失败");
            ...
            boolean checkRepairErrorFlag = b2BCenterOrderService.checkRepairError(order.getDataSourceId(), order.getOrderCondition().getOrderServiceType(), details);
            if (!checkRepairErrorFlag) {
                B2BDataSourceEnum dataSourceEnum = B2BDataSourceEnum.valueOf(order.getDataSourceId());
                return RestResultGenerator.custom(ErrorCode.ORDER_CAN_NOT_COMPLETE.code, "错误：" + dataSourceEnum.name + "维修单的上门项目没有设置维修故障，不能进行完工操作。");
            }

            restResult = orderService.saveOrderComplete(order, user, completeType, orderRequest.getBuyDate(), orderRequest.getRemarks().trim());
            return restResult;
            //return RestResultGenerator.success();
        } catch (OrderException oe) {
            return RestResultGenerator.exception(oe.getMessage());
     ...
     * app完成订单(新)
     */
    public RestResult<Object> saveAppCompleteV2(HttpServletRequest request, User user, RestCloseOrderRequest orderRequest) {
        long index = 0;
        //long index = redisUtils.incr("appClose");
        //log.warn("[[RestOrderController.saveOrderComplete]]=={}== {}",index,new Date());
        if (orderRequest == null) {
        ...
        try {
            ...
            restResult = orderService.saveOrderCompleteV2(order, user, completeType, orderRequest.getBuyDate(), orderRequest.getRemarks().trim());
            return restResult;
            //return RestResultGenerator.success();
        } catch (OrderException oe) {
            return RestResultGenerator.exception(oe.getMessage());
