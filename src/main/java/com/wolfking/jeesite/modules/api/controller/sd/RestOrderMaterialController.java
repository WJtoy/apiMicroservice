...
public class RestOrderMaterialController extends RestBaseController {
    ...
    public RestResult<Object> getProductMaterialList(HttpServletRequest request, @RequestBody RestOrderCommonRequest params) {
        ...
        try {
            ...
            if (order == null || order.getItems() == null || order.getDataSource().getIntValue() == null || order.getItems().isEmpty() || order.getOrderCondition() == null) {
                return RestResultGenerator.custom(ErrorCode.DATA_PROCESS_ERROR.code, "订单不存在，或读取订单:" + ErrorCode.DATA_PROCESS_ERROR.message);
            }
            List<RestProduct> productMaterials;
            //TODO: 云米配件必须设置yunmiCode才能显示出来
            if (order.getDataSourceId() == B2BDataSourceEnum.VIOMI.id) {
                productMaterials = appOrderMaterialService.getProductMaterialList(order.getOrderCondition().getCustomerId(), order.getItems());
            } else {
                productMaterials = appOrderMaterialService.getProductMaterialList(order.getItems());
            }
    ...
    public RestResult<Object> saveMaterialApplication(HttpServletRequest request, @RequestParam("file") MultipartFile[] files,
        ...
        try {
            ...
            master.setKefuType(condition.getKefuType());
        ...
        try {
            ...
            if (MaterialMaster.APPLY_TYPE_CHANGJIA.equals(params.getApplyType())) {
                if (StringUtils.isBlank(params.getReceiver()) || StringUtils.isBlank(params.getReceiverPhone())
                        || params.getReceiverType() == null || params.getReceiverType() == 0
                        || !params.hasReceiverAddress()) {
            ...
            String[] result;
            if (MaterialMaster.APPLY_TYPE_CHANGJIA.equals(params.getApplyType())) {
                if (params.getReceiverAreaId() != null && params.getReceiverAreaId() > 0 && StringUtils.isNotBlank(params.getReceiverSubAddress())) {
                    areaId = params.getReceiverAreaId();
                    subAddress = params.getReceiverSubAddress();
                } else {
                    try {
                        //result = AreaUtils.parseAddress(params.getReceiverAddress());  // mark on 2020-12-9
                        result = AreaUtils.parseAddressFromMS(params.getReceiverAddress());  // add on 2020-12-9
                    } catch (Exception e) {
            ...
            master.setCustomer(condition.getCustomer());
            String shopId = Optional.ofNullable(order.getB2bShop()).map(t->t.getShopId()).orElse(StrUtil.EMPTY);
            master.setShopId(StrUtil.trimToEmpty(shopId));//店铺id 2021/06/22
            master.setArea(condition.getArea());
            master.setSubArea(condition.getSubArea());
            master.setUserName(condition.getUserName());
            master.setUserPhone(condition.getServicePhone());
