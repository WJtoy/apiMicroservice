...
public class RestAuthorizeFilter extends GenericFilterBean {
    ...
    private final RedisUtilsLocal redisUtilsLocal;


    public RestAuthorizeFilter(RedisUtilsLocal redisUtilsLocal) {
    ...
    public void doFilter(final ServletRequest req, final ServletResponse res, final FilterChain chain)
        ...
        if ("OPTIONS".equals(request.getMethod())) {
            ...
            if (request.getRequestURI().toLowerCase().equals("/api/security/login") ||
                    ...
                    request.getRequestURI().toLowerCase().equals("/weChat/service/getCode") ||
                    request.getRequestURI().toLowerCase().equals("/weChat/service/wxlogin")||
                    request.getRequestURI().toLowerCase().equals("/api/config/getprojectconfigure")) {
                chain.doFilter(req, res);
            } else {
            ...
            if ((authHeader != null) && (authHeader.length() > Constant.JWT_AuthHeaderPrefix.length() + 1)) {
                ...
                try {
                    ...
                    try {
                        ...
                        String sessionValue = redisUtilsLocal.hGet(RedisConstant.RedisDBType.REDIS_NEW_APP_DB, key, "session", String.class);
