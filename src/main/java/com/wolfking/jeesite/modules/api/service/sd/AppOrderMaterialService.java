...
public class AppOrderMaterialService extends LongIDBaseService {
     ...
     * 获取产品的配件列表
     */
    public List<RestProduct> getProductMaterialList(Long customerId, List<OrderItem> orderItems) {
        List<Long> productIds = getSpecificProductIdsByOrderItems(orderItems);
        List<RestProduct> result = Lists.newArrayList();
        Map<Long, Product> productMap = productService.getProductMap(productIds);
        ...
        for (Long pId : productIds) {
            ...
            if (customerId != null && customerId > 0) {
                customerModel = productIdAndCustomerModels.stream().filter(i -> i.getName().equals(pId)).findFirst().map(NameValuePair::getValue).orElse("");
                restProduct.setMaterials(productService.getMaterialByProductId(customerId, pId, customerModel));
            } else {
                restProduct.setMaterials(Lists.newArrayList());
            }
            result.add(restProduct);
        }
        return result;
    ...
    public List<RestMaterialMasterNew> getOrderMaterials(Long orderId, String quarter) {
        List<RestMaterialMasterNew> result = org.assertj.core.util.Lists.newArrayList();
        if (orderId != null && orderId > 0 && StringUtils.isNotBlank(quarter)) {
            ...
            for (MaterialMaster master : materialMasters) {
                ...
                restMaster.setRecycleFlag(master.getRecycleFlag());
                //收件人信息
                    ...
                    for (Map.Entry<Long, List<MaterialItem>> entry : itemMap.entrySet()) {
                        ...
                        if (!entry.getValue().isEmpty()) {
                            for (MaterialItem item : entry.getValue()) {
                                ...
                                restMaterial.setRecycleFlag(item.getRecycleFlag());
                                restMaterial.setRecyclePrice(item.getRecyclePrice());
                                restMaterial.setTotalRecyclePrice(item.getTotalRecyclePrice());
