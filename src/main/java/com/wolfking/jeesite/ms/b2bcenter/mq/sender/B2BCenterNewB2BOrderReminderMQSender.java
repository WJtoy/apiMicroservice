...
@Slf4j
@Component
public class B2BCenterNewB2BOrderReminderMQSender implements RabbitTemplate.ConfirmCallback {
    ...
    @Autowired
    public B2BCenterNewB2BOrderReminderMQSender(RabbitTemplate manualRabbitTemplate, RetryTemplate kklRabbitRetryTemplate) {
    ...
    public void send(MQB2BOrderReminderMessage.B2BOrderReminderMessage message) {
        try {
            retryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                ...
                MQB2BOrderReminderMessage.B2BOrderReminderMessage msg = MQB2BOrderReminderMessage.B2BOrderReminderMessage.parseFrom((byte[]) msgObj);
                Throwable throwable = context.getLastThrowable();
                String msgJson = new JsonFormat().printToString(msg);
                B2BFailureLogUtils.saveFailureLog(B2BMQQueueType.B2BCENTER_NEW_B2BORDER_REMINDER, msgJson, 0L,
