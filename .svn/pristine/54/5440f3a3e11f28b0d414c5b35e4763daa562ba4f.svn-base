...
public class AppMyWalletController {
     ...
     * 获取网点余额信息（新）
     */
    @RequestMapping(value = "getServicePointBalanceNew", produces = "application/json;charset=UTF-8", method = RequestMethod.POST)
    public RestResult<Object> getServicePointBalanceNew(HttpServletRequest request) {
        RestLoginUserInfo userInfo = getUserInfo(request);
        if (userInfo == null || userInfo.getUserId() == null || userInfo.getServicePointId() == null) {
            return RestResultGenerator.custom(ErrorCode.LOGIN_INFO_MISSING.code, ErrorCode.LOGIN_INFO_MISSING.message);
        }
        ...
        AppGetServicePointBalanceResponse response = appMyWalletService.getServicePointBalanceNew(userInfo.getServicePointId());
        return RestResultGenerator.success(response);
    }
     ...
     * 获取网点完工金额明细(新)
     */
    @PostMapping(value = "getServicePointCompletedChargeListNew", produces = "application/json;charset=UTF-8")
    public RestResult<Object> getServicePointCompletedChargeListNew(HttpServletRequest request, @RequestBody AppGetServicePointCompletedChargeListRequest params) {
        if (params == null || params.getYearIndex() == null || params.getMonthIndex() == null) {
            return RestResultGenerator.custom(ErrorCode.WRONG_REQUEST_FORMAT.code, ErrorCode.WRONG_REQUEST_FORMAT.message);
        }
        RestLoginUserInfo userInfo = null;
        try {
            ...
            Page<EngineerChargeMaster> page = new Page<>(params.getPageNo(), params.getPageSize());
            AppGetServicePointCompletedChargeListResponse response = appMyWalletService.getServicePointCompletedChargeListNew(userInfo.getServicePointId(), params.getYearIndex(), params.getMonthIndex(), page);
            return RestResultGenerator.success(response);
        } catch (OrderException | RestAppException oe) {
            return RestResultGenerator.exception(oe.getMessage());
        } catch (Exception e) {
