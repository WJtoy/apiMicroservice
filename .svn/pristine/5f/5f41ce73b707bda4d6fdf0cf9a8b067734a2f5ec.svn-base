...
public class AreaUtils {
                ...
                if (returnStr != null && returnStr.length() > 0) {
                    ...
                    if (jsonObj.get("info").toString().toUpperCase().equals("OK")) {
                        ...
                        if (jsonArray != null && jsonArray.size() > 0) {
                            ...
                            if (redisUtilsLocal.exists(RedisConstant.RedisDBType.REDIS_SYS_AREA, String.format("2:1:%s", province.substring(0, 2)))) {
                                Long provinceId = (Long) redisUtilsLocal.get(RedisConstant.RedisDBType.REDIS_SYS_AREA, String.format("2:1:%s", province.substring(0, 2)), Long.class);
                                if (redisUtilsLocal.exists(RedisConstant.RedisDBType.REDIS_SYS_AREA, String.format("3:%d:%s", provinceId, city))) {
                                    Long cityId = (Long) redisUtilsLocal.get(RedisConstant.RedisDBType.REDIS_SYS_AREA, String.format("3:%d:%s", provinceId, city), Long.class);
                                    if (redisUtilsLocal.exists(RedisConstant.RedisDBType.REDIS_SYS_AREA, String.format("4:%d:%s", cityId, district))) {
                                        String districtString = (String) redisUtilsLocal.getString(RedisConstant.RedisDBType.REDIS_SYS_AREA, String.format("4:%d:%s", cityId, district), String.class);
                        ...
                        if (redisUtilsLocal.exists(RedisConstant.RedisDBType.REDIS_SYS_AREA, String.format("3:%d:%s", provinceId, city))) {
                            ...
                            if (redisUtilsLocal.exists(RedisConstant.RedisDBType.REDIS_SYS_AREA, String.format("4:%d:%s", cityId, district))) {
                                ...
                                if (StrUtil.isNotEmpty(street)) {
                                    ...
                                    if (redisUtilsLocal.exists(RedisConstant.RedisDBType.REDIS_SYS_AREA, String.format("5:%d:%s", districtId, street))) {
                                        String streetString = (String) redisUtilsLocal.getString(RedisConstant.RedisDBType.REDIS_SYS_AREA, String.format("5:%d:%s", districtId, street), String.class);
     ...
     *根据id获取市区县的名称
     */
    public static String getCityCountyName(String fullAreaName){
        String cityName = "";
        if (StringUtils.isNotBlank(fullAreaName)) {
            ...
            if (addressArr.length > 2) {
                ...
                cityName = StringUtils.join(temp, "");
            }
        }
        return cityName;
    }
