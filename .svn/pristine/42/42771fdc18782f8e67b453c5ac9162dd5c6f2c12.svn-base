...
public class RestServicePointController extends RestBaseController {
     ...
     * 获取是否开启互助基金
     */
    @RequestMapping(value = "getMutualFund", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
    public RestResult<Object> getMutualFund(HttpServletRequest request) {
        ...
        try {
            ...
            if (!userInfo.getPrimary()) {
                return RestResultGenerator.custom(ErrorCode.USRE_NO_ACCESS_APP.code, "您的帐号不是网点主帐号，无须阅读/开启互助基金");
            }
            ...
            RestServicePointInsuranceFlag restServicePointInsuranceFlag = appServicePointService.getInsuranceFlagByIdForAPI(userInfo.getServicePointId());
            //TODO: 停用互助基金页面的关闭按钮（不允许APP关闭互助基金）
            restServicePointInsuranceFlag.setAppCloseInsuranceFlag(true);
            return RestResultGenerator.success(restServicePointInsuranceFlag);
        } catch (Exception e) {
            ...
            return RestResultGenerator.exception("获取互助基金条款失败");
        }
     ...
     * app关闭互助基金
     */
    @RequestMapping(value = "closeMutualFund", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
    public RestResult<Object> closeMutualFund(HttpServletRequest request) {
        RestLoginUserInfo userInfo = null;
        try {
            userInfo = RestSessionUtils.getLoginUserInfoFromRestSession(request.getAttribute("sessionUserId").toString());
            if (userInfo == null || userInfo.getUserId() == null) {
            ...
            if (!userInfo.getPrimary()) {
                return RestResultGenerator.custom(ErrorCode.USRE_NO_ACCESS_APP.code, "您的帐号不是网点主帐号，不能手动关闭互助基金");
            }
            Engineer engineer = servicePointService.getEngineerFromCache(userInfo.getServicePointId(), userInfo.getEngineerId());
            if (null == engineer) {
                return RestResultGenerator.custom(ErrorCode.MEMBER_ENGINEER_NO_EXSIT.code, "读取帐号信息失败");
            ...
            RestServicePointInsuranceFlag restServicePointInsuranceFlag = appServicePointService.getInsuranceFlagByIdForAPI(userInfo.getServicePointId());
            if (!restServicePointInsuranceFlag.getAppCloseInsuranceFlag()) {
                MSErrorCode msErrorCode = appServicePointService.closeMutualFund(userInfo.getServicePointId(), userInfo.getUserId());
                if (msErrorCode.getCode() > 0) {
                    return RestResultGenerator.custom(ErrorCode.DATA_PROCESS_ERROR.code, ErrorCode.DATA_PROCESS_ERROR.message);
                }
            }
             restServicePointInsuranceFlag = appServicePointService.getInsuranceFlagByIdForAPI(userInfo.getServicePointId());
            return RestResultGenerator.success(restServicePointInsuranceFlag);
        } catch (Exception e) {
            log.error("[RestServicePointController.enableServicePointInsurance] user:{}", userInfo != null && userInfo.getUserId() != null ? userInfo.getUserId() : 0, e);
            return RestResultGenerator.exception("关闭互助基金失败");
        }
    ...
    /**
     * APP阅读且同意保险条款接口 (新 第二版)
     */
    @RequestMapping(value = "agreeServicePointCooperateClause", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
    public RestResult<Object> agreeServicePointCooperateClause(HttpServletRequest request) {
        RestLoginUserInfo userInfo = null;
        try {
            userInfo = RestSessionUtils.getLoginUserInfoFromRestSession(request.getAttribute("sessionUserId").toString());
            if (userInfo == null || userInfo.getUserId() == null) {
            ...
            user.setId(userInfo.getUserId());
            user.setName(engineer.getName());
            appServicePointService.appReadInsuranceClause(servicePoint, 10, user);
            return RestResultGenerator.success();
        } catch (Exception e) {
            log.error("[RestServicePointController.enableServicePointInsurance] user:{}", userInfo != null && userInfo.getUserId() != null ? userInfo.getUserId() : 0, e);
            return RestResultGenerator.exception("同意网点合作条款失败");
        }
    }
     ...
     * 返回app是否同意网点合作条款
     */
    @RequestMapping(value = "isAgreeServicePointCooperateClause", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
    public RestResult<Object> isAgreeServicePointCooperateClause(HttpServletRequest request) {
        RestLoginUserInfo userInfo = null;
        try {
            userInfo = RestSessionUtils.getLoginUserInfoFromRestSession(request.getAttribute("sessionUserId").toString());
            if (userInfo == null || userInfo.getUserId() == null) {
            ...
            if (!userInfo.getPrimary()) {
                return RestResultGenerator.custom(ErrorCode.USRE_NO_ACCESS_APP.code, "您的帐号不是网点主帐号，无须阅读网点合作条款");
            }
            Engineer engineer = servicePointService.getEngineerFromCache(userInfo.getServicePointId(), userInfo.getEngineerId());
            if (null == engineer) {
                return RestResultGenerator.custom(ErrorCode.MEMBER_ENGINEER_NO_EXSIT.code, "读取帐号信息失败");
            ...
            RestReadApproveInsuranceClause restReadApproveInsuranceClause = new RestReadApproveInsuranceClause();

            restReadApproveInsuranceClause.setAppServicePointClause(servicePoint.getAppInsuranceFlag());
            return RestResultGenerator.success(restReadApproveInsuranceClause);
        } catch (Exception e) {
            log.error("[RestServicePointController.readApproveInsuranceClause] user:{}", userInfo != null && userInfo.getUserId() != null ? userInfo.getUserId() : 0, e);
            return RestResultGenerator.exception("返回网点合作条款失败");
        }
    }
