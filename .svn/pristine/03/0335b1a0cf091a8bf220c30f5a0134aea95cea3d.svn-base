...
public interface MSCustomerProductFeign {
     ...
     * @return
     */
    @GetMapping("/customerProduct/getFixSpeciFromCacheForAPI")
    MSResponse<MDCustomerProduct> getFixSpecFromCacheForApi(@RequestParam("customerId") Long customerId, @RequestParam("productId") Long productId);

    /**
     * 根据客户id和产品id集合获取是否有安装规范
     * @param customerId
     * @param productIds
     * @return
     */
    @GetMapping("/customerProduct/findFixSpecByCustomerIdAndProductIdsFromCacheForAPI")
    MSResponse<Map<Long,Integer>> findFixSpecByCustomerIdAndProductIdsFromCacheForAPI(@RequestParam("customerId") Long customerId, @RequestBody List<Long> productIds);
