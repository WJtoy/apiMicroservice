...
import com.wolfking.jeesite.modules.api.entity.sd.request.RestGetReminderListRequest;
import com.wolfking.jeesite.modules.api.entity.sd.request.RestGetReminderNewRequest;
...
public class RestReminderController extends RestBaseController {
     ...
     * @return
     */
    @RequestMapping(value = "getItemsNew", produces = "application/json;charset=UTF-8", method = RequestMethod.POST)
    public RestResult<Object> getItemsNew(HttpServletRequest request, HttpServletResponse response,
                                   @RequestBody RestGetReminderNewRequest reminderRequest) {
        RestLoginUserInfo userInfo = null;
        try {
            userInfo = RestSessionUtils.getLoginUserInfoFromRestSession(request.getAttribute("sessionUserId").toString());
            if (userInfo == null || userInfo.getUserId() == null || userInfo.getServicePointId() == null) {
     ...
     * @return
     */
    @RequestMapping(value = "getItemsList", produces = "application/json;charset=UTF-8", method = RequestMethod.POST)
    public RestResult<Object> getItemsList(HttpServletRequest request, HttpServletResponse response,
                                          @RequestBody RestGetReminderListRequest reminderRequest) {
        RestLoginUserInfo userInfo = null;
        try {
            userInfo = RestSessionUtils.getLoginUserInfoFromRestSession(request.getAttribute("sessionUserId").toString());
            if (userInfo == null || userInfo.getUserId() == null || userInfo.getServicePointId() == null) {
            ...
            Reminder reminders =  reminderService.getLastReminderByOrderId(reminderRequest.getOrderId(), reminderRequest.getQuarter());
            if (CollectionUtils.isEmpty(reminders.getItems())) {
                return RestResultGenerator.custom(ErrorCode.DATA_PROCESS_ERROR.code, ErrorCode.DATA_PROCESS_ERROR.message);
            }

            return RestResultGenerator.success(reminders.getItems());
        }catch (Exception e){
            try {
                log.error("[getItems] user:{} ,json:{}", userInfo.getUserId(), GsonUtils.toGsonString(reminderRequest), e);
            } catch (Exception e1) {
