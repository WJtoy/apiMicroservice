...
import com.wolfking.jeesite.modules.sys.service.APPVerActiveService;
...
public class CommonController extends RestBaseController {
    ...
    @Autowired
    private APPVerActiveService appVerActiveService;
     ...
     * @throws Exception
     */
    @RequestMapping(value = "uploadVersionActive", method = RequestMethod.POST, produces = "application/json;charset=UTF-8")
    public RestResult<Object> uploadVersionActive(HttpServletRequest request, @RequestBody RestUploadVersionActive entity) throws Exception {
        RestLoginUserInfo restLoginUserInfo = RestSessionUtils.getLoginUserInfoFromRestSession(request.getAttribute("sessionUserId").toString());
        return appVerActiveService.saveVerActive(restLoginUserInfo.getUserId(), restLoginUserInfo.getPhoneType(), entity.getAppVersion());
    }
