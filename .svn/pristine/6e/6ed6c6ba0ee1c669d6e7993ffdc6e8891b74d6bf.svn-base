...
public class AppOrderPraiseNewService extends LongIDBaseService {
    ...
    @Autowired
    private ScheduledExecutorService orderDelayProcessExecutorService;
    ...
    public NameValuePair<Boolean, String> checkPraiseCondition(Order order, User user) {
        ...
        if(!checkSNResult){
            ...
            orderDelayProcessExecutorService.schedule(() -> {
                try {
                    b2BCenterOrderService.completeOrder(order, now, user, now);
                } catch (Exception e) {
                    log.error("[AppOrderPraiseNewService.checkPraiseCondition]：{}", Exceptions.getStackTraceAsString(e));
                }
            }, 60, TimeUnit.SECONDS);
        }
        return result;
    ...
    private void createOrderPraise(Order order, Long servicePointId, Long engineerId,
        ...
        praise.setShopId(StrUtil.trimToEmpty(shopId));//店铺id 2021/06/22
    ...
    public void updateCompleteInfo(Order order , Dict completeType , User user){
        String lockkey = null;//锁
        Boolean locked = false;
        try {
            log.error("[触发自动完工] orderId:{}",  order.getId());
            Date date = new Date();
            ...
            log.error("[AppOrderPraiseNewService.updateCompleteInfo]=={}== orderId:{}",  order.getId(), oe);
            throw oe;
        } catch (Exception e) {
            log.error("[AppOrderPraiseNewService.updateCompleteInfo]=={}== orderId:{}",  order.getId(), e);
