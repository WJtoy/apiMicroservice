...
public class Page<T> {
    ...
    private int pageSize = ApiPropertiesUtils.getPage().getPageSize();//Integer.valueOf(Global.getConfig("page.pageSize")); // 页面大小，设置为“-1”表示不进行分页（分页无效）
    ...
    public Page(int pageNo, int pageSize, int count, List<T> list) {
        ...
        this.setCount(count);
        this.list = list;
        this.pageCount = (int) (count / (pageSize < 1 ? 20 : pageSize) + this.first - 1);
        if (count % pageSize != 0 || this.pageCount == 0) {
            this.pageCount++;
        }
    }
     ...
     * @param map      本页数据对象列表
     */
    public Page(int pageNo, int pageSize, int count, Map map) {
        this.setCount(count);
        this.setPageNo(pageNo);
        this.pageSize = pageSize;
        this.map = map;
    ...
    public String toString() {
        ...
        sb.append("<input type=\"text\" value=\"" + pageNo + "\" onkeypress=\"var e=window.event||event;var c=e.keyCode||e.which;if(c==13 && this.value>0 && this.value<=");
        sb.append(pageCount + ")");
        sb.append(funcName + "(this.value," + pageSize + ",'" + funcParam + "');\" onclick=\"this.select();\"/> / ");
        sb.append("<input type=\"text\" readonly=\"readonly\" value=\"" + pageSize + "\" onkeypress=\"var e=window.event||event;var c=e.keyCode||e.which;if(c==13)");
...
//	}
    ...
    public void setAppPageCount(int pageCount) {
        this.pageCount = pageCount;
    }

    public void setAppRowCount(int rowCount) {
        this.count = rowCount;
    }
    ...
    public String getMapString() {
        ...
        if (pageNo == first) {// 如果是首页
            ...
            //sb.append("<li><a href=\"javascript:\" onclick=\""+funcName+"("+prev+","+pageSize+",'"+funcParam+"');\">&#171; 上一页</a></li>\n");
            sb.append("<span><a class=\"pageLink\"href=\"javascript:\" onclick=\"" + funcName + "(" + prev + "," + pageSize + ",'" + funcParam + "');\">&#171; 上一页</a></li>\n");
        }
        ...
        if (begin > first) {
            ...
            for (i = first; i < first + slider && i < begin; i++) {
                //sb.append("<li><a href=\"javascript:\" onclick=\""+funcName+"("+i+","+pageSize+",'"+funcParam+"');\">"
                //        + (i + 1 - first) + "</a></li>\n");
                sb.append("<span><a class=\"pageLink\" href=\"javascript:\" onclick=\"" + funcName + "(" + i + "," + pageSize + ",'" + funcParam + "');\">"
                        + (i + 1 - first) + "</a></li>\n");
            }
            if (i < begin) {
                // sb.append("<li class=\"disabled\"><a href=\"javascript:\">...</a></li>\n");
...
//            sb.append("<li><a href=\"javascript:\" onclick=\""+funcName+"("+next+","+pageSize+",'"+funcParam+"');\">"
//                    + "下一页 &#187;</a></li>\n");
            sb.append("<span><a class=\"pageLink\" href=\"javascript:\" onclick=\"" + funcName + "(" + next + "," + pageSize + ",'" + funcParam + "');\">"
                    + "下一页 &#187;</a></span>\n");
        }
